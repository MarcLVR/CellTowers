...
# Filter networks of interest
networks <- names(network_colors)
towers_filtered <- towers_near_nile_df %>%
...
# Add network layers
for (net in networks) {
  net_data <- towers_filtered %>% filter(Network == net)
  ...
  map <- map %>%
    ...
    addCircleMarkers(
      ...
      popup = ~paste0(
        ...
        "<br><b>Radio:</b> ", radio,
        "<br><b>CID:</b> ", CID,
        "<br><b>Range:</b> ", RANGE, " m"
      ),
      group = net
    )
}

# Add interactive toggle for networks
map <- map %>%
  addLayersControl(
    overlayGroups = networks,
